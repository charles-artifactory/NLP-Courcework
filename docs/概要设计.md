...
系统采用分层模块化架构，src目录下按功能划分为多个子目录：

| 模块分类 | 模块名称 | 文件路径 | 职责 |
|---------|---------|---------|------|
| 配置 | 配置管理 | src/config.py | 系统配置参数管理 |
| 处理层 | 文档处理 | src/processing/document_processor.py | 文档解析与分块 |
| 检索层 | 嵌入模块 | src/retrieval/embedder.py | 文本向量化与存储 |
| 检索层 | 检索模块 | src/retrieval/retriever.py | 稀疏/稠密/混合检索 |
| 核心层 | 生成模块 | src/core/generator.py | LLM答案生成 |
| 核心层 | RAG流水线 | src/core/pipeline.py | 流程编排与对话管理 |
| 服务层 | API服务 | src/api/server.py | RESTful HTTP接口 |
| 应用层 | 前端应用 | app/gradio_app.py | Web交互界面 |
...
为避免不相关查询（如"你好"）返回无关文档，实现多层过滤：

```mermaid
flowchart LR
    R[检索结果] --> PF[预过滤]
    PF --> |原始相似度>=0.5| N[归一化融合]
    N --> RR[重排序]
    RR --> FF[最终过滤]
    FF --> |融合分数>=0.25 且 rerank>0| O[输出结果]
```

**过滤策略**：
1. **预过滤**：原始余弦相似度 >= 0.5（在归一化前过滤）
2. **最终过滤**：同时满足融合分数 >= 0.25 和重排序分数 > 0

### 7.6 LLM动态配置（创新点）

支持运行时切换LLM提供商，无需重启应用：

```python
# 支持的提供商
providers = ["ollama", "openai"]

# OpenAI兼容API支持（DeepSeek、Azure等）
client = OpenAI(
    api_key=api_key,
    base_url=custom_base_url  # 如 https://api.deepseek.com/v1
)
```

### 7.7 无知识库对话模式（创新点）

当检索结果为空或全部被过滤时，自动切换为普通对话模式：

```python
if search_results:
    system_prompt = RAG_SYSTEM_PROMPT  # RAG模式
else:
    system_prompt = CHAT_SYSTEM_PROMPT  # 普通对话模式
```
